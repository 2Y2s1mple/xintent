...
public class QueryIntentHook extends XC_MethodHook {
    ...
    protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
        ...
        String log = String.format("%s { calling=[%d]%s:%d, resolvedType=%s, flags=%s, intent=%s }\n",
                methodName,
                userId,
        ...
        if (stackElements != null) {
            for (int i = 3; i < stackElements.length; i++) {
                String clz = stackElements[i].getClassName();
                String mtd = stackElements[i].getMethodName();
...
//                        + "----" + stackElements[i].getMethodName());
                ...
                if (clz.toLowerCase().contains("xposed") && mtd.equals("handleHookedMethod")) {
                    return;
                }
            }
        }
