...
public class StartServiceLockedHook extends XC_MethodHook {
    ...
    // Lcom/android/server/am/ActiveServices;->startServiceLocked(Landroid/app/IApplicationThread;Landroid/content/Intent;Ljava/lang/String;IIZLjava/lang/String;Ljava/lang/String;IZ)Landroid/content/ComponentName;
    @Override
    protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
        ...
        // ComponentName startServiceLocked(IApplicationThread caller0, Intent service1, String resolvedType2,
            // int callingPid3, int callingUid4, boolean fgRequired5, String callingPackage6, String callingFeatureId7, int userId8, boolean allowBackgroundActivityStarts9)
        ...
        String callingFeatureId7 = (String) args[7];
        int userId8 = (int) args[8];
        boolean allowBackgroundActivityStarts9 = (boolean) args[9];
        ...
        String log;
        if (xb == null) {
            log = String.format("startService { callingPkg=[%d]%s:%d-%d, rType=%s, fgRequired=%b, cFID=%s, allowBgA=%b, retCmp=%s, target=%s, intent=%s }\n",
                    userId8, callingPackage, callingUid, callingPid,
                    resolvedType2,
                    requireForeground,
                    callingFeatureId7,
                    allowBackgroundActivityStarts9,
                    ret,
                    tcmp,
                    intent.toString()
            );
        } else {
            log = String.format("startService { callingPkg=[%d]%s:%d-%d, rType=%s, fgRequired=%b, cFID=%s, allowBgA=%b, retCmp=%s, target=%s, intent=%s, extras=%s}\n",
                    userId8, callingPackage, callingUid, callingPid,
                    resolvedType2,
                    requireForeground,
                    callingFeatureId7,
