...
import com.zwk.xintent.MyXposed;
...
public class BroadcastIntentLockedHook extends XC_MethodHook {
    ...
    // Lcom/android/server/am/ActivityManagerService;->broadcastIntentLocked(Lcom/android/server/am/ProcessRecord;Ljava/lang/String;Landroid/content/Intent;Ljava/lang/String;Landroid/content/IIntentReceiver;ILjava/lang/String;Landroid/os/Bundle;[Ljava/lang/String;ILandroid/os/Bundle;ZZIIIIIZ)I
    @Override
    protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
        ...
        // broadcastIntentLocked(ProcessRecord callerApp0, String callerPackage1, Intent intent2, String resolvedType3, IIntentReceiver resultTo4, int resultCode5,
            ...
            // int callingPid13, int callingUid14, int realCallingUid15, int realCallingPid16, int userId17, boolean allowBackgroundActivityStarts18) {
        ...
        int realCallingPid16 = (int) args[16]; // from PI sendInner
        boolean allowBackgroundActivityStarts18 = (boolean) args[18];
        ...
        String log;
        if (xb == null) {
            log = String.format("sendBroadcast { calling=%s:%d:%d, rc=%d:%d, rType=%s, rCode=%d, rData=%s, rExtras=%s, " +
                            "BgStart=%b, appOp=%d, ordered=%b, sticky=%b, target=%s, rpermission=%s, intent=%s }\n",
                    callerPackage, callingUid14, callingPid13,
                    realCallingUid15, realCallingPid16,
                    resolvedType, resultCode, resultData, resultExtras,
                    allowBackgroundActivityStarts18, appOp9, ordered11, sticky12,
                    tcmp,
            ...
            log = String.format("sendBroadcast { calling=%s:%d:%d, rc=%d:%d, rType=%s, rCode=%d, rData=%s, rExtras=%s, " +
                            "BgStart=%b, appOp=%d, ordered=%b, sticky=%b, target=%s, rpermission=%s, intent=%s, extras=%s }\n",
                    callerPackage, callingUid14, callingPid13,
                    realCallingUid15, realCallingPid16,
                    resolvedType, resultCode, resultData, resultExtras,
                    allowBackgroundActivityStarts18, appOp9, ordered11, sticky12,
